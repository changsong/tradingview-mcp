...
async function main() {
  ...
  for (const symbol of symbols) {
    ...
    try {
      ...
      await sleep(8000); // 等待图表加载

      // 设置5分钟周期（中国市场用5分钟更合适）
      runCommand('timeframe 5');
      await sleep(5000); // 等待周期切换
  ...
  profitable.forEach((r, idx) => {
    ...
    console.log(`   26周位置: ${pos}%, 5日波动: ${vol}%, 日线趋势: ${r.dailyTrend}\n`);
  });
  ...
  // 保存结果
  const outputFile = `batch_backtest_cn_v8_${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
  writeFileSync(outputFile, JSON.stringify({
    test_time: new Date().toISOString(),
    strategy: 'CN-急涨急跌吸筹策略 v8 (精准入场+防早止损)',
