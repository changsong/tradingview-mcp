#!/usr/bin/env node
/**
 * v22 batch test — 30 symbols from cn.txt
 * v22 changes: atrMult 2.0→1.5, atrMultAfterTP1 1.8→1.2, tp2Mult 3.0→3.5
 */
...
const v22src = readFileSync(path.join(__dir, 'A_Share_SQZMOM_PRO_v22_Daily.pine'), 'utf8');
...
console.log(`\n=== v22 Batch Test (${symbols.length} symbols) ===`);
console.log('v22: 年度区间上半段过滤 (aboveAnnualMid)\n');

// Set Pine source
console.log('Setting Pine source (v22)...');
const setResult = TVstdin('pine set', v22src);
...
console.log('Adding v22 to chart...');
let chartLoaded = false;
for (let attempt = 0; attempt < 5; attempt++) {
  TVargs(['ui', 'eval', clickJS], 15000);
  await sleep(3000);
  const ns = TV('state');
  if (ns?.studies?.some(s => s.name.includes('v22'))) {
    console.log(`  v22 confirmed on chart!`);
    chartLoaded = true;
    break;
  }
  if (attempt < 4) {
...
console.log('\n' + '='.repeat(72));
console.log('v22 vs v19 汇总');
console.log('='.repeat(72));
console.log(`有交易股票: v19=${v19w.length}/30  v22=${w.length}/30`);
console.log(`均值交易数: v19=${(v19T/v19w.length).toFixed(1)}  v22=${avgT.toFixed(1)}  Δ${(avgT-v19T/v19w.length)>=0?'+':''}${(avgT-v19T/v19w.length).toFixed(1)}`);
console.log(`综合胜率:   v19=${v19WR.toFixed(1)}%  v22=${avgWR.toFixed(1)}%  Δ${(avgWR-v19WR)>=0?'+':''}${(avgWR-v19WR).toFixed(1)}%`);
console.log(`均值净收益: v19=${v19Net.toFixed(2)}%  v22=${avgNet.toFixed(2)}%  Δ${(avgNet-v19Net)>=0?'+':''}${(avgNet-v19Net).toFixed(2)}%`);
console.log(`均值最大DD: v19=${v19DD.toFixed(2)}%  v22=${avgDD.toFixed(2)}%`);
console.log(`均值PF:     v22=${avgPF.toFixed(3)}`);
...
console.log(`\nWR>=50%: v19=${v19w.filter(r=>r.winRate>=50).length}/${v19w.length}  v22=${wr50}/${w.length}`);
console.log(`WR>=60%: v19=${v19w.filter(r=>r.winRate>=60).length}/${v19w.length}  v22=${wr60}/${w.length}`);
console.log(`PF>=1.2: v22=${pf12}/${w.length}  PF>=1.5: v22=${pf15}/${w.length}`);
...
improved.sort((a,b) => (b.netPct-(v19map[b.symbol]?.netPct||0)) - (a.netPct-(v19map[a.symbol]?.netPct||0))).slice(0,8).forEach(r => {
  ...
  console.log(`  ${r.symbol.padEnd(16)} v19=${(v.netPct||0).toFixed(1)}% → v22=${r.netPct.toFixed(1)}% Δ${(r.netPct-(v.netPct||0))>=0?'+':''}${(r.netPct-(v.netPct||0)).toFixed(1)}%`);
});
...
regressed.sort((a,b) => (a.netPct-(v19map[a.symbol]?.netPct||0)) - (b.netPct-(v19map[b.symbol]?.netPct||0))).slice(0,8).forEach(r => {
  const v = v19map[r.symbol] || {};
  console.log(`  ${r.symbol.padEnd(16)} v19=${(v.netPct||0).toFixed(1)}% → v22=${r.netPct.toFixed(1)}% Δ${(r.netPct-(v.netPct||0)).toFixed(1)}%`);
});

const outPath = path.join(__dir, 'v22_batch_30.json');
