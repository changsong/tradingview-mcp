#!/usr/bin/env node
/**
 * 港股多周期技术面深度分析 — Master v2 (准确率优化版)
 *
 * 与 analyze_tech_cn_mtf.mjs 结构一致；港股专属调整：
 *   • 移除 A 股 ±10/20% 涨跌停板检测（港股蓝筹无每日硬性涨跌停）
 *   • benchmark 改为 TVC:HSI（恒生指数）
 *   • 周期权重沿用 US 配比（HK 是 T+0 市场，短周期信号更重要）
 *
 * 指标：EMA5/20/50/200, RSI, MACD, ADX(14), Squeeze(BB+KC), ATR, OBV,
 *       Pivot HH/HL, Stoch RSI, RSI 背离, 量能高潮, 收益加速度, R/R, RS vs HSI
 *
 ...
 *
 * 用法：
 *   node analyze_tech_hk_mtf.mjs                           # 读 hk_selected.txt
 *   node analyze_tech_hk_mtf.mjs --symbols=HKEX:700,HKEX:2018
 *   node analyze_tech_hk_mtf.mjs --top=10                  # 按新闻分前 N 只
 *   node analyze_tech_hk_mtf.mjs --no-live                 # 关闭 chart 实时值校准
 *   node analyze_tech_hk_mtf.mjs --no-bench                # 跳过 RS vs HSI
 */
...
const OUT_MD    = './watchlist/hk_tech_signals.md';
const OUT_JSON  = './watchlist/hk_tech_signals.json';
const SEL_TXT   = './watchlist/hk_selected.txt';
const NEWS_MD   = './watchlist/hk_news_signals.md';
const NEWS_JSON = './watchlist/hk_news_signals.json';
const BENCH_SYM = 'TVC:HSI';   // 恒生指数
...
function loadSymbols(args) {
  if (args.symbols) return args.symbols.split(',').map(s => s.trim()).filter(Boolean);
...
function loadNewsScores() {
  ...
  const map = {};
  // HK 符号格式：HKEX:700 — 与 CN 不同：CN 是 SSE/SZSE:6位数
  const re = /\|\s*\d+\s*\|\s*([^|]+?)\s*\|\s*([A-Z]+:[A-Z0-9]+)\s*\|\s*\*\*([+\-]?\d+)\*\*\s*\|\s*([^|]+?)\s*\|/g;
  let m;
  while ((m = re.exec(raw))) {
    const name = m[1].trim();
    const sym  = m[2].trim();
...
function scoreTF(bars, tfKey, live = {}, extra = {}) {
  ...
  if (extra.rs != null && tfKey === '1D') {
    if (extra.rs > 15)       { score += 10; notes.push(`RS+${extra.rs}% 大幅跑赢HSI(+10)★★`); }
    else if (extra.rs > 5)   { score += 6;  notes.push(`RS+${extra.rs}% 跑赢HSI(+6)★`); }
    else if (extra.rs > 0)   { score += 3;  notes.push(`RS+${extra.rs}% 略强HSI(+3)`); }
    else if (extra.rs < -5)  { score -= 4;  notes.push(`RS${extra.rs}% 跑输HSI(-4)`); }
  }
...
function buildReport(results, opts) {
  ...
  p('# 港股多周期技术面分析 · 明日最优入场报告 (v2)');
  p(`**日期:** ${now.slice(0, 10)}　**周期权重:** 1W(25%)+1D(40%)+4H(25%)+1H(10%)`);
  p(`**股票池:** ${results.length} 只　**数据源:** hk_selected.txt${opts.haveNews ? ' + hk_news_signals.md' : ''}${opts.haveBench ? ' + 恒生指数(HSI)基准' : ''}`);
  p('');

  p('## 📊 综合技术评分总览');
  p('');
  p('| # | 名称 | 代码 | 综合(1W/1D/4H/1H) | 对齐 | ADX | RS | R/R | 闻 | 终值 | 信号 | 信心 | 类型 | 追涨 | SQZMOM | 风险 |');
  p('|---|------|------|------------------|------|-----|----|----|----|------|------|------|------|------|--------|------|');
  results.forEach((r, i) => {
    ...
    p(`| ${i+1} | ${r.name} | ${r.symbol} | ${tc} (${sc}) | ${r.alignment.label} | ${adx} | ${rs} | ${rr} | ${r.newsScore != null ? (r.newsScore >= 0 ? '+'+r.newsScore : r.newsScore) : '-'} | **${fc}** | **${r.signal.bias}** | **${r.signal.confidence}** | ${r.signal.type} | ${r.signal.chaseOK} | ${r.sqzmom} | ${risk} |`);
  });
  p('');
  p('---');
  p('');
  ...
  if (best.length === 0) {
    ...
    best.forEach((r, i) => {
      ...
      if (d) {
        ...
        p(`| RS vs HSI | ${d.rs != null ? (d.rs >= 0 ? '+'+d.rs : d.rs) + '%' : '-'} |`);
  ...
  p(`*增强: ADX 震荡过滤 / 背离检测 / 量能高潮 / Squeeze 持续 / 收益加速 / 支撑次数 / R/R + ATR 止损 / RS-vs-HSI*`);
  p(`*港股专属: T+0 当日可回转，无涨跌停硬限制；价格以 HKD 计价*`);
  return L.join('\n');
}
...
function detail(r, rank) {
  ...
  if (d) {
    ...
    p(`| RS vs HSI | ${d.rs != null ? (d.rs >= 0 ? '+'+d.rs : d.rs) + '%' : '-'} | ${d.rs > 5 ? '领涨' : d.rs > 0 ? '跑赢' : d.rs < -5 ? '跑输' : '-'} |`);
...
async function main() {
  ...
  console.log('\n╔══════════════════════════════════════════════════════════╗');
  console.log('║  港股多周期技术分析 (v2 准确率优化版)                    ║');
  console.log('║  EMA/RSI/MACD/ADX/Squeeze/OBV + StochRSI + 背离          ║');
  console.log('║  + 量能高潮/Squeeze持续/收益加速/支撑/RR/RS-vs-HSI       ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
  ...
  results.slice(0, 30).forEach((r, i) => {
    ...
    console.log(
      ...
      `综:${tc}  终:${fc}  ADX:${adx}  RS:${rs}  对齐:${r.alignment.label.padEnd(11)} ` +
